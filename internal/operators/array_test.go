...
func TestArrayOperator_ToSQL(t *testing.T) {
	...
	tests := []struct {
		...
		// All tests
		{
			...
			expected: "NOT EXISTS (SELECT 1 FROM UNNEST([10 20 30]) AS elem WHERE NOT (item > 5))",
			hasError: false,
		},
		{
			...
			expected: "NOT EXISTS (SELECT 1 FROM UNNEST(ages) AS elem WHERE NOT (item >= 18))",
			hasError: false,
		},
		{
		...
		// Some tests
		{
			...
			expected: "EXISTS (SELECT 1 FROM UNNEST([1 2 3 4 5]) AS elem WHERE item = 3)",
			hasError: false,
		},
		{
			...
			expected: "EXISTS (SELECT 1 FROM UNNEST(statuses) AS elem WHERE item = 'active')",
			hasError: false,
		},
		{
		...
		// None tests
		{
			...
			expected: "NOT EXISTS (SELECT 1 FROM UNNEST([1 2 3 4 5]) AS elem WHERE item < 0)",
			hasError: false,
		},
		{
			...
			expected: "NOT EXISTS (SELECT 1 FROM UNNEST(values) AS elem WHERE item = 'invalid')",
			hasError: false,
		},
		{
